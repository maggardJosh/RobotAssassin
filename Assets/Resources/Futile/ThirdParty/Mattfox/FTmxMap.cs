...
public class FTmxMap : FContainer
{
    ...
    virtual protected FTilemap createTilemap(XMLNode node)
    {
        ...
        if (node.attributes.ContainsKey("visible"))
            tilemap.isVisible = int.Parse(node.attributes["visible"]) != 0;
        if (node.attributes.ContainsKey("opacity"))
            tilemap.alpha = float.Parse(node.attributes["opacity"]);
    ...
    virtual protected FNode createTileObject(XMLNode node)
    {
        ...
        // assemble whole name
        string name = baseName + "/" + actualFrame;
