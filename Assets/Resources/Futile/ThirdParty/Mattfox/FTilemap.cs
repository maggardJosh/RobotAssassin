...
public class FTilemap : FContainer {
			...
			foreach (FSprite tile in _tiles) {
				...
				if (tileChangedX || tileChangedY) {
					...
					if (frame >= 0 && frame < _tileArray.GetLength(0)) {
						...
						tile.element = Futile.atlasManager.GetElementWithName(_baseName+"/"+frameNum);
						tile.isVisible = true;
					} else {
	...
	public void LoadText (string text, bool skipZero=true) {
		...
		FAtlasElement element =	Futile.atlasManager.GetElementWithName(_baseName+"/1");
		...
		if (!_skipZero) {
			...
			if (_tiles.Count <= 0) {
				for (int i = 0; i < totalNeeded; i++) {
					FSprite sprite = new FSprite(_baseName + "/1"); // set to 1
				...
				for (int i = start; i < totalNeeded; i++) {
						FSprite sprite = new FSprite(_baseName + "/1");
						sprite.shader = _shader;
			...
			for (int j = 0; j < clipTilesHigh; j++) {
				...
				if (!_skipZero || frame > 0) {
					...
					if (_skipZero) {
						...
						sprite.element = Futile.atlasManager.GetElementWithName(_baseName+"/"+frame);
					}
